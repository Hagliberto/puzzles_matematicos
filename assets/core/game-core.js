...
function createDragDropGame(options){
  ...
  const solutionMode=localStorage.getItem("jogosMagicosSolutionMode")==="1";
  const solutionData=(window.MAGIC_CHALLENGES||[]).find(g=>g.id===options.gameId)?.solution||options.solution||null;
  if(solutionMode) document.body.classList.add("solution-mode");
  const themeBtn=document.getElementById("themeToggle"); if(themeBtn)themeBtn.addEventListener("click",toggleTheme);
  if(!solutionMode&&!isUnlocked(options.gameId)){location.href="../../index.html";return}
  ...
  function savePaused(markPaused=true){if(solutionMode||state.won)return;const db=normalizeDB();if(hasProgress())db.paused[options.gameId]={slots:state.slots,elapsed:state.elapsed,savedAt:new Date().toISOString(),active:markPaused};writeDB(db)}
